...
MzTreeView.prototype.focus = function(sourceId, defer)
{
  ...
  if(path[0]!="0")
  {
    alert("Node "+ sourceId +" Without the correct tree node anchored on the effective！\r\n"+
...
MzTreeView.prototype.expandAll = function()
{
  if(this.totalNode>500) if(
    confirm("Do you want to stop to start all nodes? \ r \ n \ r \ n nodes too much! Expand the time-consuming")) return;
...
MzTreeView.prototype.Click = function(sourceId, defer)
{
  ...
  if(path[0]!="0")
  {
    alert("Node" + sourceId + "is not affiliated to effectively correct tree node! \ r \ n" +
      "Node id sequence = "+ path.join(this.divider));
    return;
  }
  var root = this.node["0"], len = path.length;
