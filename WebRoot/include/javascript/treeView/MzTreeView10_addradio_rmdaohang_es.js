...
function MzTreeView(Tname)
{
  if(typeof(Tname) != "string" || Tname == "")
    throw(new Error('Construcción de una instancia de clase cuando la instancia de la clase, por favor consulte el nombre de variable pasada!'));
...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var ctrlValue  = this.getAttribute(source, "ctrlValue");      //zengchuan modify on 2007-4-17
  ...
  this.node[this.index] =
  {
    ...
    "ctrlValue" : ctrlValue     	//zengchuan modify on 2007-4-17
  };
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(node.ctrl){
  	HTML += "<input type='"+ node.ctrlType +"' name='"+ node.ctrlName +"' value='"+ node.ctrlValue + "' "
  	+ " id='"+ this.name +"_checbox_"+ id +"' style='height:15px;' " 
  	+ " onclick=\""+ this.name +".checkChild('"+ id +"')\" ";	
  ...
  HTML += "<IMG "+
    ...
    "target='"+ target +"' "+
    "title='"+ node.hint +"' "+
    //"onfocus=\""+ this.name +".focusLink('"+ id +"','"+this.name+"_link_"+ id +"')\" "+
...
//焦点聚集到树里的节点链接时的处理
//id 客户端节点 id
MzTreeView.prototype.focusLink= function(id,selfid)
{
  window.parent.parent.frames["table_index"].frames["login_index"].document.getElementById("main_t_m").innerHTML = "Cargo actual:";
  var aobject = document.getElementById(selfid);
  var imglengt = aobject.parentNode.firstChild.getElementsByTagName("IMG").length;
  var selfinnerhtml = aobject.innerHTML;

  var navdiv = window.parent.parent.frames["table_index"].frames["login_index"].document.getElementById("main_t_r");
  navdiv.innerHTML = "<span class='firstspan'>&nbsp;</span>";
  var spanEle = window.parent.parent.frames["table_index"].frames["login_index"].document.createElement("span");						
  var textAll = selfinnerhtml;
  var spanText = window.parent.parent.frames["table_index"].frames["login_index"].document.createTextNode(textAll);						
  spanEle.appendChild(spanText);												
  navdiv.appendChild(spanEle);
  
  var shangji = aobject;  	
  for(i=0; i<imglengt; i++){
  	if(shangji.parentNode.parentNode.parentNode.parentNode.nodeName == "SPAN"){
	  	var shangaobject = shangji.parentNode.parentNode.parentNode.parentNode.previousSibling.getElementsByTagName("A")[0];
	  	var shangainnerhtml = shangaobject.innerHTML;
	  	
	  	shangji = shangaobject;	  	
	  	
	  	var spanEle = window.parent.parent.frames["table_index"].frames["login_index"].document.createElement("span");						
		var textAll = shangainnerhtml;
		var spanText = window.parent.parent.frames["table_index"].frames["login_index"].document.createTextNode(textAll);						
		spanEle.appendChild(spanText);
		
		var spanes = navdiv.getElementsByTagName("SPAN");		
		navdiv.insertBefore(spanEle,spanes[1]);
	  }
  }
  ...
  if (area)
  {
    ...
    if(!node.isLoad)
    {
      ...
      if(node.hasChild && node.childNodes.length>200)
      {
        ...
        this.colors.highLightText +"; font-size: 9pt'>Por favor espere ...</A></NOBR></DIV>";
      }
...
MzTreeView.prototype.focus = function(sourceId, defer)
{
  ...
  {
    alert("nodo "+ sourceId +" Sin el nodo del árbol correcto anclado en efectivo!\r\n"+
      "nodo id secuencia = "+ path.join(this.divider));
    return;
  }
...
MzTreeView.prototype.expandAll = function()
{
  if(this.totalNode>500) if(
    confirm("¿Quieres dejar de empezar a todos los nodos? \r\n\r\n Nodo demasiado! Ampliar el tiempo")) return;
...
MzTreeView.prototype.checkChild = function(id)
{
	...
	if(tcn.ctrlType!="radio"){ //yuanpf 2011-09-09 11:12 增加radio判断
		this.selChild(id);
		this.selParent(id);
	}else{
		this.focusClientNode(id);
		securityRoleVsMenus_pageSearch();
	}
