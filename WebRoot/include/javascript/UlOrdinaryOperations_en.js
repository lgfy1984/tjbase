...
function fHrefToUrlWithTip(verbId){
	if(confirm("Really need to abandon the current operation?")){
		var base = document.all.tags('BASE')[0].href;
		var action = document.forms[0].action;
		var href = "";
		if(action.substring(0, base.length)===base){
...
function goPage2(){			  
  ...
  var _total=document.getElementById('_total');
    	...
    	if(parseInt(_tp.value)>parseInt(_total.value)){
      		alert("Can not be greater than the total number of pages!");
      		return;
    	} 
...
function isInteger(tip, oElm){
	if(!isMadeOf(oElm.value,'1234567890')){
   		alert("Error ["+ tip +"] entry, please re-entry");
   		oElm.value="";
   		oElm.focus();
   		return;
  	}
...
function fSimpleSavingSubmit(){	
		...
		for(var i=0; i<inputs.length; i++){											
			...
			if(type=="TEXT"){//input.text
				if(ulmust==true && isNull(input)){
					//alert(msg);
					alert(ultip + "can not be empty, please input!");
					return false;
				}
				if(ullen!=-1){
					if(ullen<lengthOfString(value)){
						//alert(msg);
						alert(ultip + "input of length can not exceed" + ullen + ", has entered the length of" + value.length);
						return false;
					}
				}
				if(ultype=="INT"){
					if(ulmust==true && isNull(input) && !isDigit(value)){
						//alert(msg);
					 	alert(ultip + "value must be entered.");
						return false;
					}
				}
				if(ultype=="DATE"){
					if(ulmust==true && isNull(input) && !testSimpleDate(value)){
						//alert(msg);
					 	alert(ultip + "must be entered a valid date format \ n format: yyyy-MM-dd");
						return false;
					}
				}
				if(ultype=="FLOAT"){
					try{
						if(checkFloatNum(value)==false){
							alert(ultip + "integer or fractional, please fill in the correct");
							return false;						
						}
						var f = parseFloat(value);
						if(f>99999999){
							alert(ultip + "not more than 99,999,999");
							return false;
						}
					}catch(e){
						alert(ultip + "integer or fractional, please fill in the correct");
	...
	if(textareas!=null && textareas.length>0){
		for(var i=0; i<textareas.length; i++){
			...
			msg += ", ullen="+len;
			if(must==true && value==""){
				alert(tip+"Can not be empty, please input!");
				//alert(msg);
				return false;
			}
			if(len!=-1){
				if(len<lengthOfString(value)){
					alert(tip + "input of length can not exceed" + len);
				//	alert(msg);
					return false;
				}
			}
			if(type=="INT"){
				if(must==true && value=="" && !isDigit(value)){
				 	alert(tip + "value must be entered.");
					//alert(msg);
					return false;
				}
			}
	...
	if(selects!=null && selects.length>0){
		for(var i=0; i<selects.length; i++){
			...
			if(len!=-1){
				if(len<lengthOfString(value)){
					alert(p + "input of length can not exceed" + len);
					return false;
				}
			}
			if(type=="INT"){
...
function getUlmust(oElm){
	...
	if(must=="TRUE"){
		...
		return false;
	 	...
	 	throw new Error(oElm.name+"的自定义属性ulmust必须为true或者false");	
	}
}
...
function getUllen(oElm){
	...
	if(!isDigit(len)){
	 	throw new Error(oElm.name + "custom attributes ullen must be an integer value.");
	}
	return parseInt(len);
}
function lTrim(str){
		...
		for(var i=0; i<length; i=i+1){
			if(i==0){
				...
				if(tagName!=oElm[i].tagName.toUpperCase()){
					throw(new Error(-1, "Not the same type of control!"))
				}
			}
		}
		if(tagName=="INPUT"){
...
function textInputLimit(){
	...
	for(var i=0; i<count; i++){
		...
		if(input.type.toUpperCase()=="TEXT"){
			...
			if(val==""){//录入为空
				if(nul=="1"){
					alert(des + "can not be empty, please fill in!");
					//alert(msg);
					return false;
				}
			}else{//录入非空
				...
				if(length>eff){//长度检测
					alert(des + "longer than" + eff + ", please check!");
					//alert(msg);
					return false;
				}
			}
