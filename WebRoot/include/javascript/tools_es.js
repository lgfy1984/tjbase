...
//      return false;
    if(oneChar<"0"||oneChar>"9"){
      alert("Parte '"+ strNombre +"' sólo se puede rellenar un entero mayor que 0");
      return false;
    }
  }
  return true;
...
function emailVerify(objEmail,blnEmpty,strName){
  ...
  if((emailAdd==null || emailAdd=="") && blnEmpty==false){
	  alert("Parte '"+ strNombre +"' no puede estar vacío.");
	  objEmail.focus();
	  return false;
	  }
 if( emailAdd!="" && !check_email(emailAdd)){
	  	alert("Parte '"+ strNombre +"' es incorrecto \ n Por favor, consulte el siguiente formato: zonghua@126.com");
		objEmail.focus();
		return false;
	  }
  return true;
...
//校验电话号码录入的有效性,有效返回true,无效返回false
	...
	if(strText=="" && blnEmpty==false ){
		alert("Parte '"+ strNombre +" debe ser llenado!");
	  	objText.focus();
      	return false;
	}else if(strText!=""){
		if(strText.length > intLength){
			alert("Parte '"+ strNombre +"' excede la longitud especificada ("+ intLength +" caracteres), por favor, corríjanme.");
	  		objText.focus();
      		return false;
		}
		sStr=/[0-9]/g;
		...
		if(bln==null){
			alert("Parte '"+ strNombre +"' requerido por los números!");
	  		objText.focus()
      		return false;
		}
	}
  ...
  for(i=0;i<strText.length;i++){
    if(strText.charCodeAt(i)!=32){
      if(strText.charCodeAt(i)<65 || ( strText.charCodeAt(i)>90 && strText.charCodeAt(i)<97 )  ||  strText.charCodeAt(i)>122 ){
        alert("Parte '"+ strNombre +"' sólo se puede llenar en las letras.");
        objText.focus();
        return false;
      }
    }
  ...
  for(i=0;i<strText.length;i++){
    if(strText.charCodeAt(i)!=32){
      if(strText.charCodeAt(i)<48 || ( strText.charCodeAt(i)>57 && strText.charCodeAt(i)<65 )  || ( strText.charCodeAt(i)>90 && strText.charCodeAt(i)<97 )  ||  strText.charCodeAt(i)>122 ){
        alert("Parte '"+ strNombre +"' sólo se puede llenar en las letras y números.");
        objText.focus();
        return false;
      }
    }
...
//校验纯数字文本录入的有效性,有效返回true,无效返回false
  ...
  for(i=0;i<strText.length;i++){
    if((strText.charCodeAt(i)<48 || strText.charCodeAt(i)>57)){
      alert("Parte '"+ strNombre +"' Por favor, use sólo números.");
      objText.focus();
      return false;
    }
  }
...
//校验整数文本录入的有效性,有效返回true,无效返回false
  ...
  for(i=0;i<strText.length;i++){
    if((strText.charCodeAt(i)<48 || strText.charCodeAt(i)>57)){
      alert("Parte '"+ strNombre +"' sólo se puede llenar un número entero.");
      objText.focus();
      return false;
    }
  }
...
//校验带小数数字文本录入的有效性,有效返回true,无效返回false
  ...
  if( point==-1){
    if(strText.length > totalLen-decimalLen){
      alert("Parte "+ strNombre +" Introduzca el número es demasiado grande, correcto.");
      objText.focus();
      return false;
    }
  }else{
    ...
    if(strText.length-(point+1) > decimalLen){
      alert("Parte '"+ strNombre +" Introduzca el número de decimales sólo puede tener" +  decimalLen + "poco, por favor, corríjanme.");
      objText.focus();
      return false;
    }
  }
...
//校验关联日期的有效性,有效返回true,无效返回false
  ...
  if(blnEmpty){
    ...
    if(strStart==""){
      alert("Parte '"+ strEndName +"' no puede estar vacío.");
      objStart.focus();
      return false;
    }
    if(strEnd==""){
  ...
  if(intStart>intEnd){
    alert("Parte '"+ strStartName +"' fecha especificada, no más que una parte '"+ strEndName +"' noche de la fecha especificada.");
    objStart.focus();
    return false;
  }else{
    return true;
...
if(blnNowtimeAfter && !(nowDate - 24*3600000 <= inputDate)){
			alert("¡Lo siento! La sección \"" + strNombre + "\" entrar en el tiempo necesario de lo que es ya tarde");
			objname.focus();
			return false;
		}
if(!blnNowtimeAfter && !(nowDate>=inputDate)){
			alert("¡Lo siento! La sección \"" + strNombre + "\" tiempo de entrada antes de lo que ahora es necesario")
			objname.focus();
			return false;
	}
return true;
...
function selectVerify(objText,strName){
  ...
  if(objText != null){
    ...
    alert("parte'"+ StrNombre +"' no existe.");
    return false;
  }
}
...
//校验日期的有效性,有效返回true,无效返回false
  ...
  if(strText.length!=strFormat.length){
    alert("Parte '"+ strNombre +"' no está formateado correctamente, siga los "+ strFormat +" 'formato de la fecha de entrada.");
    objText.focus();
    return false;
  }
  ...
  for(i=0;i<strText.length;){
	  ...
	  if(strFormatTemp=="MM" && (strTemp>12 || strTemp<1)){
          alert("Parte '"+ strNombre +"' del mes de forma incorrecta, siga el "+ strFormat +" 'formato de la fecha de entrada.");
          objText.focus();
          return false;
	  }
	  if((strFormatTemp=="dd" || strFormatTemp=="DD")  && (strTemp>31 || strTemp<1)){
          alert("Parte '"+ strNombre +"' fecha de entrada no es correcta, de acuerdo con "+ strFormat +" 'formato de la fecha de entrada.");
          objText.focus();
          return false;
	  }
	  if(strFormatTemp=="hh" && (strTemp>24 || strTemp<0)){
          alert("Parte '"+ strNombre +"' La hora de forma incorrecta, siga el "+ strFormat +" 'formato de la fecha de entrada.");
          objText.focus();
          return false;
	  }
	  if((strFormatTemp=="mm" || strFormatTemp=="ss" ) && (strTemp>59 || strTemp<0)){
          alert("Parte '"+ strNombre +"' de minutos o segundos de forma incorrecta, siga el "+ strFormat +" 'formato de la fecha de entrada.");
          objText.focus();
          return false;
	  }
  	  i=i+2;
...
function initBusiMgrSelect(selectObjName, empArray, defaultValue){
    ...
    var curEmpSelect = document.getElementById(selectObjName);
       ...
       curEmpSelect[0] = new Option("--Por favor, seleccione el gerente de negocios--","");
...
function SelectOrgTreeVerify(strName,blnEmpty,objname){
	...
	if(deptObj!=null && empStr!=null){
		...
		alert("Parte '"+ strNombre +"' no puede estar vacío, por favor seleccione el personal apropiado！‘");
...
function SelectOrgTree(default_empId,objname,arrayName,btnSelect,default_area,default_dept){
	...
	areaObj[0]=new Option("--Seleccione la empresa--","");
	...
	//初始化部门列表
	deptObj[0]=new Option("--Seleccione el departamento--","");
	...
	//初始化人员列表；
	empObj[0]=new Option("--Por favor, seleccione al personal--","");
...
function SelectOrgTreeAll(default_empId,objname,arrayName){
		...
		areaObj[0]=new Option("--Seleccione por favor--","");
...
function percentVerify(objname,blnPassHundred,strName){
		...
		if(objname.value/100>1 && !blnPassHundred ){
				alert("La sección \"" + strNombre + "\" valor de entrada no puede superar el 100%");
			objname.focus();
			return false;
		}
		return true;
