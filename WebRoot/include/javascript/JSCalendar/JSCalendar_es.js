...
/**
  ...
  */
var _sNeededFilePath = "/hollycrm/script/JSCalendar/";
...
function createCldTabFrm(){
	...
	_TD.innerHTML = "<table cellspacing=0 cellpadding=0 class=calendar style='border:0px solid;width:100%'>"
		+ "<tr><td title=   'la fecha de hoy'style='cursor:hand'onclick=\"parent._bHaveSelectNewValue=true;parent.setTargetFormaValue(" + _dftD.getFullYear() + "," + (_dftD.getMonth() + 1) + "," + _dftD.getDate() + ");parent.hideCldTabFrm();return;parent.fillCldTabFrm(" + _dftD.getFullYear() + "," + _dftD.getMonth() + "," + _dftD.getDate() + ");\">"
		+ "<b>&nbsp;<img src='" + _imgBgToday1.src + "' width=30px>hoy : " + _dftD.getFullYear() + "-" + (_dftD.getMonth() + 1) + "-" + _dftD.getDate()
		+ "</td>"
		+ "<td width=30px>"
		+ "<img style='cursor:hand' onclick=parent.resetTargetValue() title='Fecha de limpieza' src='" + _imgReset1.src + "' onmouseover=this.src='" + _imgReset2.src + "' onmouseout=this.src='" + _imgReset1.src + "'></td>"
...
//_months[_dCurMonth]
	...
	_sTimes="<table  border='0' cellpadding='0' cellspacing='0'>"
		...
		+"<td rowspan='2' align='center' class='title'>horas</td>"
		...
		+"<td rowspan='2' align='center'  class='title'>minutos</td>"
...
function whenMouseOverDateItem(){
	...
	if(e.tagName == "TD"){
		...
		e.title = "la fecha actual: " + sCurDate;
...
function checkTime(obj){
	...
	if((tValue>59 || tValue<0 || tValue=="" || !isNum(tValue))&& obj== tMin  ){
			alert("El formato de su introducción no es correcto, revise");
			tMin.focus();
	}
	if((tValue>23 || tValue<0 || tValue=="" || !isNum(tValue))&& obj== tHour ){
			alert("您所输入的小时格式不对,请检查")
			tHour.focus();
			}	
	}
