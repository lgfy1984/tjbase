...
function tjsubmit(formName,action,op,flag)
{
	...
	if(flag!=null && typeof(flag)=="boolean")
	{
		...
		if(num<1)
		{
			alert("seleccione los registros que quiere operar！");
			return false;
		}
		if(!flag && num>1)
		{
			alert("Esta operación sólo permite seleccionar un registro！");
			return false;
		}

		if(op=="del")
		{
			if(!window.confirm("Seguro de que deseas borrar el  "+num+"registros?"))
				return false;
		}
