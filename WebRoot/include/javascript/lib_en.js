...
function tjsubmit(formName,action,op,flag)
{
	...
	if(flag!=null && typeof(flag)=="boolean")
	{
		...
		if(num<1)
		{
			alert("Please select the operating records!");
			return false;
		}
		if(!flag && num>1)
		{
			alert("This charge only select a record promise!");
			return false;
		}

		if(op=="del")
		{
			if(!window.confirm("Sure you want to delete the selected "+num+"records ?"))
				return false;
		}
