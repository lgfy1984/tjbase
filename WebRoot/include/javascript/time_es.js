...
function fDrawCal(iYear, iMonth, iCellHeight, iDateTextSize) {
  ...
  with (document) {
	write("<tr>");
	for(i=0; i<7; i++)
		write("<td "+styleTD+"color:#cc6600' >" + WeekDay[i] + "</td>");
	write("</tr>");
...
write("<iframe  style='position:absolute;z-index:-1;width:100%;height:100%;top:0;left:0;scrolling:no;' frameborder='0' src='about:blank'></iframe>");
write("<table border='0' bgcolor='#EEFDEF'>");
write("<TR>");
write("<td valign='middle' align='center'><input type='button' name='PrevMonth' value='<' style='height:20;width:20;FONT:bold' onClick='fPrevMonth()'>");
write("&nbsp;<SELECT name='tbSelYear' onChange='fUpdateCal(tbSelYear.value, tbSelMonth.value)' Victor='Won'>");
for(i=1910;i<2030;i++)
	write("<OPTION value='"+i+"'>"+i+"años</OPTION>");
write("</SELECT>");
...
write("<DIV style='background-color:#BBF0D1'><table width='100%' border='0' cellpadding='2'>");
fDrawCal(giYear, giMonth, 12, 12);
write("</table></DIV>");
write("</td>");
write("</TR><TR><TD align='center'>");
write("<B style='cursor:hand; font: bold 14 Times New Roman' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=0'>hoy:"+giYear+"año"+giMonth+"mes"+giDay+"día</B>");
write("</TD></TR>");
write("</TABLE></Div>");
...
function arrowtag(namestr,valuestr,size){
   istr=unescape('%u25bc');          
   document.write("<input type='text' name='"+namestr+"' readonly value='"+valuestr+"' size='"+size+"' class='s01'><input style='font-size:8pt;font-family:Times New Roman;' type='button' value='"+istr+"'onclick='fPopCalendar("+namestr+","+namestr+");return false'>");
  }
