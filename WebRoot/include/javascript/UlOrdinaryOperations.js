		...
		for(var i=0; i<inputs.length; i++){											
			...
			if(type=="TEXT"){//input.text
				if(ulmust==true && isNull(input)){
					//alert(msg);
					alert(ultip+"不能为空，请录入！");
					return false;
				}
				if(ullen!=-1){
					if(ullen<lengthOfString(value)){
						//alert(msg);
						alert(ultip+"的录入长度不能超过"+ullen+"，已录入长度"+value.length);
						return false;
					}
				}
				if(ultype=="INT"){
				...
				if(ultype=="FLOAT"){
					try{
						if(checkFloatNum(value)==false){
							alert(ultip+"为整数或小数，请正确填写");
							return false;						
						}
						var f = parseFloat(value);
						if(f>99999999){
							alert(ultip+"不能超过99999999");
							return false;
						}
					}catch(e){
						alert(ultip+"为整数或小数，请正确填写");
...
function getUllen(oElm){
	...
	if(!isDigit(len)){
	 	throw new Error(oElm.name+"的自定义属性ullen必须为整数数值");
	}
	return parseInt(len);
}
function lTrim(str){
