...
<script language="javascript">
	function selectThis(value1,value2,value3,value4,value5,value6,hspstaffid){
	...
	function select(value1,value2,value3,value4,value5,value6,hspstaffid){
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/include/css/open_ frame_add.css" />
...
<script language="javascript" src="<bean:message key="security.js.gettext_staff.path" bundle="security" />"  ></script>
	...
	<body>
	<form name="form" method="post" action="<%=request.getContextPath()%>/hsp/hspStaffBaseinfo.do">
	...
	<table align="center" width="570" style="background-color:#828283; margin-top:10px;" cellspacing="1" cellpadding="0" border="0"> 
		...
		<tr>
			<td class="date_img">
					<!--<bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.itemName"/>-->机构：
					<input type="hidden" id="hiddenInputId_1" name="hspConfigBaseinfoId" value="${dataForm.hspConfigBaseinfoId}"/>
					<input type="text" id="displayInputId_1" name="hspConfigName" readonly="readonly" value="<%=dataForm.getHspConfigName() %>" onkeydown="huiche()" />
					...
					<!--<bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.idNo"/>-->证件：<input name="idNo" id="idNo" value="${dataForm.idNo}" " type="text"  onkeydown="if(event.keyCode==13){event.keyCode=9;document.getElementById('queryImg').className='queryImg_down'}" />	
	...
	<table align="center" class="list_table" width="570" cellspacing="1" cellpadding="0" border="0"> 
		<tr class="tdnav">
			...
			<td width="100">
				<bean:message  bundle="hsp.hspLocale" key="hsp.jsp.hspstaffbaseinfolist.sex"/>
			</td>
			<!--<td width="100">
		...
		<tr <%=i%2==0?"class='trColor_white'" : "class='trColor_defale'"%>
			onclick="select('<%=dataForm.getHspIdArray()[i] %>','<%=dataForm.getHspNameArray()[i] %>','<%=dataForm.getNameArray()[i] %>','<%=dataForm.getIdNoArray()[i] %>','<%=dataForm.getCommSexIdArray()[i] %>','<%=dataForm.getDateOfBirthArray()[i] %>','<%=dataForm.getIdsHiddenArray()[i] %>');tresOn(this)" 
			ondblclick="selectThis('<%=dataForm.getHspIdArray()[i] %>','<%=dataForm.getHspNameArray()[i]  %>','<%=dataForm.getNameArray()[i] %>','<%=dataForm.getIdNoArray()[i] %>','<%=dataForm.getCommSexIdArray()[i] %>','<%=dataForm.getDateOfBirthArray()[i] %>','<%=dataForm.getIdsHiddenArray()[i] %>')" 
			onmouseover="overColor(this)"
			>
			<td class="leftPadding "><%=dataForm.getNameArray()[i]%></td>
			<td class="leftPadding "><%=dataForm.getSexArray()[i]%></td>
			<!--<td class="leftPadding "><%=dataForm.getHspNameArray()[i]%></td>-->
			<td class="leftPadding "><%=dataForm.getIdNoArray()[i]%></td>
		</tr>
		...
		<tr>
			<td colspan="3" align="center" bgcolor="#ffffff" height="35px">
				<%
					int curPage = 0;
					int totalNum = 0;
					int pageSize = 0;
