	...
	调用ajax
	*/
    function findNames(url,displayInputId,hiddenInputId,code1111) {
        ...
        default :    
			...
			*/
	        ...
	        if(completeBody.rows.length>0){
	            ...
	            XMLHttpReq.onreadystatechange = function(){processMatchResponse(displayInputId,hiddenInputId,code1111)};  //指定对象状态判断的方法
	...
	//对象状态判断的方法
    function processMatchResponse(displayInputId,hiddenInputId,code1111) {
    	
    	if (XMLHttpReq.readyState == 4) { // readyState　为正常返回的时候
        	if (XMLHttpReq.status == 200) {　　//成功响应 
        		     
				  			
                setNames(XMLHttpReq.responseXML,displayInputId,hiddenInputId,code1111); 
	...
	*/
     function setNames(names,displayInputId,hiddenInputId,code1111) {            
       ...
       　　　　　　　　　　　　　　　　　　　　//设置显示位置
        ...
        for(var i=0 ;i < elementroot.length;i++){
            ...
            row.onclick = function(){
				...
				document.getElementById("itemCode_value").value = this.getElementsByTagName('td')[0].innerHTML;
			...
			row.ondblclick = function(){
				if(code1111==1){
					...
					var itemCode=document.getElementById("itemCode_value").value
					var values=(itemCode+empNo).substring((itemCode+empNo).length-6);
					document.getElementById("empNo").value=itemCode+values;
				}
...
*/
	function closed(){
		...
		windowstate = false;
		if(nplace!=null&&nplace!=""){
			document.getElementById(nplace).focus();
		}
		if(divlayer != ""){
			document.getElementById(divlayer).style.display = "block";
		}
		var check=path.substring((path.length-12),path.length);
		if(check=="getAllPeople"){
			document.getElementById("checkisorno").value="getAllPeople";
...
选中处理
*/
	function xuanzss(displayInputId,hiddenInputId,code1111){
			if(code1111==1){
		    	var empNo=document.getElementById("empNo").value;
				var itemCode=document.getElementById("itemCode_value").value
				var values=(itemCode+empNo).substring((itemCode+empNo).length-6);
...
//					var empNo=document.getElementById("empNo").value;
//					item_code=document.getElementById("itemCode_value").value;
//					alert(item_code);
//					var values=(item_code+empNo).substring((item_code+empNo).length-6);
//					document.getElementById("empNo").value=item_code+values;	
	...
	*****/
	function add(url,displayInputId,hiddenInputId,divlevel,place,code1111){	
			path=url;
			nplace=place;
			if(divlevel != null&&divlevel!=""){
...
//			}
			...
			*/
			var check=path.substring((path.length-12),path.length);
			if(check=="getAllPeople"){
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">Phonetic code</option><option value="2">Number of Resident(PMI)</option><option value="3">los caracteres chinos</option></select>Query:<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="itemCode_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" maxlength="30" /><input type="button" class="button" value="selected" onclick="xuanzss(\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="115">Number of Resident(PMI)</td><td width="85">Phonetic code</td><td width="136">Name</td></tr><tbody id="guoji_body"></tbody></table></div>';	
			}
			else{
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">Phonetic code</option><option value="2">Code</option><option value="3">los caracteres chinos</option></select>Query:<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="itemCode_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" maxlength="30" /><input type="button" class="button" value="selected" onclick="xuanzss(\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">Code</td><td width="90">Phonetic code</td><td width="166">Name</td></tr><tbody id="guoji_body"></tbody></table></div>';		
			}
			fade("body_div");//调用该函数作用是背景渐变透明	
			
			findNames(url,displayInputId,hiddenInputId,code1111);
