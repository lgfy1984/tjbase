     ...
     function setNames(names,displayInputId,hiddenInputId) {            
       ...
       　　　　　　　　　　　　　　　　　　　　//设置显示位置
        ...
        for(var i=0 ;i < elementroot.length;i++){
        	...
        	var resItemName = "";
        	var resItemNameXML =ress.getElementsByTagName("resItemName")[0]; 
        	if(resItemNameXML.childNodes[0] != null){
        		resItemName = ress.getElementsByTagName("resItemName")[0].firstChild.data;
        	};
            ...
            row.onclick = function(){
				...
				document.getElementById("input_value").value = this.getElementsByTagName('td')[0].innerHTML;
			...
			row.ondblclick = function(){
				...
				document.getElementById(displayInputId).value = this.getElementsByTagName('td')[0].innerHTML;
...
//				
//			}else{
				fulldiv.style.width= document.body.offsetWidth;
				fulldiv.style.height= document.body.offsetHeight;
				newele.style.width= document.body.offsetWidth;
				newele.style.height= window.screen.availHeight;
				document.getElementById("bgiframe").style.width= document.body.offsetWidth;
				document.getElementById("bgiframe").style.height= document.body.offsetHeight;				
//			}
			...
			bgdiv.style.left=(bodywidth-385)/2;		
			...
			*/
			div_pinyin.innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">el código fonético</option><option value="2">PMI</option><option value="3">los caracteres chinos</option></select>la condición de consulta:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\');" maxlength="30" /><input type="button" value="seleccionando" onclick="xuanz(\''+displayInputId+'\',\''+hiddenInputId+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">nombre</td><td width="90">sexos</td><td width="166">fecha de nacimiento</td></tr><tbody id="guoji_body"></tbody></table></div>';		
