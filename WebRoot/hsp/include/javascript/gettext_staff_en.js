...
//			}
			...
			*/
			var check=path.substring((path.length-12),path.length);
			if(check=="getAllPeople"){
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">Phonetic code</option><option value="2">Number of Resident(PMI)</option><option value="3">los caracteres chinos</option></select>Query:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\');" maxlength="30" /><input type="button" class="button" value="selected" onclick="xuanz(\''+displayInputId+'\',\''+hiddenInputId+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="115">Number of Resident(PMI)</td><td width="85">Phonetic code</td><td width="136">Name</td></tr><tbody id="guoji_body"></tbody></table></div>';	
			}
			else{
