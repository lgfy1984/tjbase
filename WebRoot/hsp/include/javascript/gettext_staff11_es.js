    ...
    function processMatchResponse(displayInputId,hiddenInputId,code1111) {
    	...
    	if (XMLHttpReq.readyState == 4) { // readyState　为正常返回的时候
        	if (XMLHttpReq.status == 200) {　　//成功响应 
                ...
                window.alert("los datos de recuperar están fracasos!");
            }
        }
...
//			}
			...
			*/
			var check=path.substring((path.length-12),path.length);
			if(check=="getAllPeople"){
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">código fonético</option><option value="2">N º residentes(PMI)</option><option value="3">los caracteres chinos</option></select>condición de consulta:<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="itemCode_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" maxlength="30" /><input type="button"  value="seleccionando" onclick="xuanzss(\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="115">N º residentes(PMI)</td><td width="85">código fonético</td><td width="136">nombre</td></tr><tbody id="guoji_body"></tbody></table></div>';	
			}
			else{
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">código fonético</option><option value="2">código</option><option value="3">los caracteres chinos</option></select>condición de consulta:<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="itemCode_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" maxlength="30" /><input type="button"  value="seleccionando" onclick="xuanzss(\''+displayInputId+'\',\''+hiddenInputId+'\',\''+code1111+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">código</td><td width="90">código fonético</td><td width="166">nombre</td></tr><tbody id="guoji_body"></tbody></table></div>';		
			}
