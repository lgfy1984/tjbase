...
function checkGender(){
        ...
        if(idNumber!=null){
         if(strsex%2==0){
            ...
            function showMessage(status,message,staffName){
		        if(status==1){		            
		                var left = self.parent.frames["menuFrame"];
		                left.modifyNode(staffName);
		                alert("保存成功！");	            
		        }
		    }
</script>
  </head>
  <body onload="showMessage('<%=data.getStatus() %>','<%=data.getMessage() %>','<%=data.getName() %>');">
  	 <form name="form" action="<%=request.getContextPath()%>/hsp/orgMenu.do" 
  	 	method="post" enctype="multipart/form-data"> 
  	 	<input type="hidden" id="verbId" name="verbId" value="staffUpdate" />
  	 	<input type="hidden" id="idHidden" name="idHidden" value="${data.id}" />
			...
			<tr>
				...
				<td class="tblLable"><span>*</span><bean:message  bundle="hsp.hspLocale" key="hsp.jsp.hspstaffbaseinfodetail.organ"/>：</td>
  				<td style="padding-right:40px;">
					...
					<input type="hidden" id="hiddenInputId_1" name="hspConfigBaseinfoId" value="${data.hspConfigBaseinfoId}"/>
  				</td>
  			...
  			<tr>
  				...
  				value="<%=data.getName() %>" onkeypress="eventOnKeyPress('idNo')" onkeydown="huiche()" onfocus="getInfomation()" maxlength="50"/> 						
  				</td>
  				<td class="tblLable"><span>*</span><bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.idNo"/>：</td>
  				<td>
  					<input type="text" id="idNo" name="idNo" style="font-size:12px" value="<%=data.getIdNo() %>" onkeydown="huiche()"  onblur="setTime()"
  			...
  			<tr>
  				...
  				<!-- 	<input type="text" onfocus="setTime()" name="birthdayYear" id="birthdayYear" value="<%=data.getBirthdayYear() %>" onkeypress="eventOnKeyPress('birthdayMonth')" onkeyup="value=value.replace(/[^(\d\.)]/g,'')" onblur="check4year('birthdayYear');" class="input" style="width:40px;" onkeydown="huiche()" maxlength="4"/>年
  				 ...
  				 -->
  				 <input type="text" name="birthdayYear" style="font-size:12px" readonly="readonly" id="birthdayYear" value="<%=data.getBirthdayYear() %>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d'})" onkeydown="huiche()" maxlength="10"/>
  				</td>
  			...
  			<tr>
				...
				<td style="padding-right:40px;">
				    <input type="text" id="displayInputId_2" value="<%=data.getTheNationalName() %>"
				    	onkeypress="eventOnKeyPress('officeTel')" name="commConfigNationalityId" onkeydown="huiche()" readonly />
					<input type="hidden" id="hiddenInputId_2" value="<%=data.getNationalityIdHidden() %>" name="nationalityIdHidden" />
  				...
  				<td>
  				<input type="hidden" id="hiddenInputId_3" name="securityUserBaseinfoId" value="<%=data.getSecurityUserBaseinfoId() %>"  />
  			...
  			<tr>
  				...
  				<td>
  					<input type="text" value="<%=data.getOfficeTel() %>" 
  			...
  			<tr>
  				...
  				<td>
  					<input type="text" name="workCertificateNo" style="font-size:12px"  value="<%=data.getWorkCertificateNo() %>" onkeypress="eventOnKeyPress('islocation')"
  				...
  				<td>
  					<select name="islocation" style="font-size:12px" onkeydown="huiche()" onkeypress="eventOnKeyPress('commDictPublicCharId1')">			
  						...
  						<%if(data.getIslocation()!=null&&data.getIslocation()>=0){ %>
  							<option value="1" <%out.print(data.getIslocation()==1?"selected":""); %> ><bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.on"/></option>
  							<option value="0" <%out.print(data.getIslocation()==0?"selected":""); %> ><bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.notIn"/></option>
  						<%}else{ %>
  			...
  			<tr>
  					...
  					<td>
  						<select  onkeypress="eventOnKeyPress('commDictPublicCharId2')" style="font-size:12px"  name="commDictPublicCharId1" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommDictPublicCharId1().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
  				...
  				<td>
  					<select onkeypress="eventOnKeyPress('commConfigPositiontitleId')" style="font-size:12px" name="commDictPublicCharId2" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommDictPublicCharId2().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  			...
  			<tr>
  				<td class="tblLable"><bean:message  bundle="hsp.hspLocale" key="hsp.jsp.common.managePost"/></td>
  					<td>
  						<select onkeypress="eventOnKeyPress('commConfigEmptitleId')" style="font-size:12px" name="commConfigPositiontitleId" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommConfigPositiontitleId().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  				...
  				<td>
  					<select onkeypress="eventOnKeyPress('commDictPublicCharId3')" style="font-size:12px" name="commConfigEmptitleId" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getItemCode() %>" <%out.print(data.getCommConfigEmptitleId().trim().equals(nCommDictPublicChar.getItemCode())?"selected":""); %> >
  			...
  			<tr>
  					...
  					<td>
  						<select onkeypress="eventOnKeyPress('commConfigDegreeId')" style="font-size:12px" name="commDictPublicCharId3" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommDictPublicCharId3().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  				...
  				<td>
  					<select onkeypress="eventOnKeyPress('commConfigDegreeLevelId')" style="font-size:12px"  name="commConfigDegreeId" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommConfigDegreeId().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  			...
  			<tr>
  					...
  					<td>
  						<select onkeypress="eventOnKeyPress('commConfigProfessionId')" style="font-size:12px" name="commConfigDegreeLevelId" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommConfigDegreeLevelId().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  				...
  				<td>
  					<select onkeypress="eventOnKeyPress('startWorkDateYear')" style="font-size:12px" name="commConfigProfessionId" onkeydown="if(event.keyCode==13)event.keyCode=9">
							...
							%>
											<option value="<%=nCommDictPublicChar.getId() %>" <%out.print(data.getCommConfigProfessionId().trim().equals(nCommDictPublicChar.getId())?"selected":""); %> >
												<%=nCommDictPublicChar.getDictValue() %>
											</option>
							<%										
									}
  			...
  			<tr>
  				...
  				<td>
  					...
  					value="<%=data.getStartWorkDateYear() %>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d'})"  onkeydown="huiche()" maxlength="10"/>
  				</td>
