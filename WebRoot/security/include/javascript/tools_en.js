...
function emailVerify(objEmail,blnEmpty,strName){
 ...
 if( emailAdd!="" && !check_email(emailAdd)){
	  	alert("Part‘"+strName+"’format is not correct! \n Please refer to the following format:zonghua@126.com");
		objEmail.focus();
		return false;
	  }
  return true;
...
//校验电话号码录入的有效性,有效返回true,无效返回false
	...
	if(strText=="" && blnEmpty==false ){
		alert("Part‘"+strName+"’must fill in!");
	  	objText.focus();
      	return false;
	}else if(strText!=""){
		if(strText.length > intLength){
			alert("Part‘"+strName+"’than the specified length("+intLength+"character)，Please correct。");
	  		objText.focus();
      		return false;
		}
		sStr=/[0-9]/g;
		...
		if(bln==null){
			alert("Part‘"+strName+"’necessary by the numbers!");
	  		objText.focus()
      		return false;
		}
	}
...
//校验纯数字文本录入的有效性,有效返回true,无效返回false
  ...
  for(i=0;i<strText.length;i++){
    if((strText.charCodeAt(i)<48 || strText.charCodeAt(i)>57)){
      alert("Part‘"+strName+"’can only fill in the integer。");
      objText.focus();
      return false;
    }
  }
...
//校验带小数数字文本录入的有效性,有效返回true,无效返回false
  ...
  if( point==-1){
    if(strText.length > totalLen-decimalLen){
      alert("Part‘"+strName+"’enter the number is too large, please correct。");
      objText.focus();
      return false;
    }
  }else{
    ...
    if(strText.length-(point+1) > decimalLen){
      alert("Part‘"+strName+"’Enter the number of decimal places only "+decimalLen+"-bit，please correct。");
      objText.focus();
      return false;
    }
  }
...
//校验关联日期的有效性,有效返回true,无效返回false
  ...
  if(intStart>intEnd){
    alert("Part‘"+strStartName+"’than the specified date can not be part‘"+strEndName+"’later date specified。");
    objStart.focus();
    return false;
  }else{
    return true;
...
if(blnNowtimeAfter && !(nowDate - 24*3600000 <= inputDate)){
			alert("Sorry! Part\""+strName+"\" time entered late than it is now necessary");
			objname.focus();
			return false;
		}
if(!blnNowtimeAfter && !(nowDate>=inputDate)){
			alert("Sorry! Part\""+strName+"\" time entered earlier than it is now necessary")
			objname.focus();
			return false;
	}
return true;
...
//校验日期的有效性,有效返回true,无效返回false
  ...
  if(strText.length!=strFormat.length){
    alert("Part‘"+strName+"’of the format is not correct, follow‘"+strFormat+"’ enter the date format 。");
    objText.focus();
    return false;
  }
  ...
  for(i=0;i<strText.length;){
	  ...
	  if(strFormatTemp=="MM" && (strTemp>12 || strTemp<1)){
          alert("Part‘"+strName+"’ of the month is entered incorrectly, follow‘"+strFormat+"’ enter the date format 。");
          objText.focus(); 
          return false;
	  }
	  if((strFormatTemp=="dd" || strFormatTemp=="DD")  && (strTemp>31 || strTemp<1)){
          alert("Part‘"+strName+"’ of the date is entered incorrectly, follow‘"+strFormat+"’ enter the date format 。");
          objText.focus();
          return false;
	  }
	  if(strFormatTemp=="hh" && (strTemp>24 || strTemp<0)){
          alert("Part‘"+strName+"’ of the hour entered is not correct, follow‘"+strFormat+"’ enter the date format 。");
          objText.focus();
          return false;
	  }
	  if((strFormatTemp=="mm" || strFormatTemp=="ss" ) && (strTemp>59 || strTemp<0)){
          alert("Part‘"+strName+"’of the minutes or seconds, the input is not correct, follow‘"+strFormat+"’ enter the date format 。");
          objText.focus();
          return false;
	  }
  	  i=i+2;
...
function areaId_OnChange(objname,arrayName){
	...
	areaObj=document.getElementById(areaStr);
	deptObj=document.getElementById(deptStr);
	deptObj[0]=new Option("--Please select department--","");
...
function percentVerify(objname,blnPassHundred,strName){
		...
		if(objname.value/100>1 && !blnPassHundred ){
				alert("Column\""+strName+"\"input value can not exceed 100%");
			objname.focus();
			return false;
		}
		return true;
