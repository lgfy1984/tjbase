...
function MzTreeView(Tname)
{
  if(typeof(Tname) != "string" || Tname == "")
    throw(new Error('Por favor cree una instancia de clase, cuando una referencia al nombre de la clase variable de instancia pasó!'));
...
{
  var main = window.parent.parent.frames["table_index"].frames["login_index"].document.getElementById("main_t_m");
  main.innerHTML = "";
  ...
  var selfinnerhtml = aobject.innerHTML;
	...
	selfinnerhtml = selfinnerhtml.replace(/<BR>/g,' ').replace(/<br>/g,' ');
  ...
  //var textAll = selfinnerhtml.substr(0,60); //60个词之内能显示
  ...
  for(i=0; i<imglengt; i++){
  	if(shangji.parentNode.parentNode.parentNode.parentNode.nodeName == "SPAN"){
	  	...
	  	shangainnerhtml = shangainnerhtml.replace(/<BR>/g,' ').replace(/<br>/g,' ');
