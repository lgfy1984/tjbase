...
function MzTreeView(Tname)
{
  if(typeof(Tname) != "string" || Tname == "")
    throw(new Error('Please create a class instance, when a reference to the class instance variable name passed!'));
...
{
  window.parent.parent.frames["table_index"].frames["login_index"].document.getElementById("main_t_m").innerHTML = "Current position:：";
...
MzTreeView.prototype.focus = function(sourceId, defer)
{
  ...
  {
    alert("Node "+ sourceId +" Without the correct tree node anchored on effective!\r\n"+
      "Node id sequence = "+ path.join(this.divider));
    return;
  }
...
MzTreeView.prototype.expandAll = function()
{
  if(this.totalNode>500) if(
    confirm("Do you want to stop to start all nodes?\r\n\r\nNode too much! Expand the time-consuming")) return;
