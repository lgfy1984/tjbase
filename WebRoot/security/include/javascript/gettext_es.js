     ...
     function setNames(names,displayInputId,hiddenInputId) {            
       ...
       　　　　　　　　　　　　　　　　　　　　//设置显示位置
        ...
        for(var i=0 ;i < elementroot.length;i++){
        	...
        	var resInputCode = "";
        	var resInputCodeXML = ress.getElementsByTagName("resInputCode")[0];
			if(resInputCodeXML.childNodes[0] != null && resInputCodeXML.childNodes[0].nodeValue != null){
				resInputCode = resInputCodeXML.childNodes[0].nodeValue;
				
			}
			
        	var resItemCode = "";
        	var resItemCodeXML = ress.getElementsByTagName("resItemCode")[0];
			if(resItemCodeXML.childNodes[0] != null && resItemCodeXML.childNodes[0].nodeValue != null){
				resItemCode = resItemCodeXML.childNodes[0].nodeValue;				
			}
			
        	var resItemName = "";
        	var resItemNameXML = ress.getElementsByTagName("resItemName")[0];
			if(resItemNameXML.childNodes[0] != null && resItemNameXML.childNodes[0].nodeValue != null){
				resItemName = resItemNameXML.childNodes[0].nodeValue;
			}
...
*/
	...
	*****/
	function add(url,displayInputId,hiddenInputId,divlevel,nextitemfocus){
			if(divlevel != null){
				document.getElementById(divlevel).style.display = "none";
				divlayer = divlevel;
			}
			if(nextitemfocus != null){
				nextitem = nextitemfocus;
			}
...
//			}
			...
			*/
			document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">código fonético</option><option value="2">código</option><option value="3">nombre</option></select>pregunta:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\');" maxlength="30" /><input type="button"  value="seleccionado" onclick="xuanz(\''+displayInputId+'\',\''+hiddenInputId+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">código</td><td width="90">código fonético</td><td width="166">nombre</td></tr><tbody id="guoji_body"></tbody></table></div>';		
