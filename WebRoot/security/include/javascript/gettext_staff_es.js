        ...
        for(var i=0 ;i < elementroot.length;i++){
        	...
        	row.onmouseover = function(){
				...
				for(i=0; i<completeBody.rows.length; i++){			
					//if(i!=currentLine){
						if(i == rowNumber){		
							completeBody.rows[i].style.background = "#D2EEFF";		
							completeBody.rows[i].style.cursor="pointer";							
							//completeBody.rows[i].className = "trColor_over";			
						}else{		
							completeBody.rows[i].style.background = "white";				
            ...
            row.onclick = function(){
				...
				for(i=0; i<completeBody.rows.length; i++){			
					if(i == rowNumber){		
						completeBody.rows[i].style.background="#F1F7FC";							
...
*/
	...
	*****/
	function add(url,displayInputId,hiddenInputId,divlevel){	
...
//			}
			...
			*/
			...
			if(check=="getPeople"){
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">código fonético</option><option value="2">Personal de salud código</option><option value="3">nombre</option></select>pregunta:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\');" maxlength="30" /><input type="button" value="seleccionado" onclick="xuanz(\''+displayInputId+'\',\''+hiddenInputId+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="">Personal de salud código</td><td width="">código fonético</td><td width="">nombre</td></tr><tbody id="guoji_body"></tbody></table></div>';		
			}else{
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:70px;margin-right:10px;"><option value="1">código fonético</option><option value="2">código</option><option value="3">nombre</option></select>pregunta:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+displayInputId+'\',\''+hiddenInputId+'\');" maxlength="30" />&nbsp;<input type="button" width="90px" value="seleccionado" onclick="xuanz(\''+displayInputId+'\',\''+hiddenInputId+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="">código</td><td width="90">código fonético</td><td width="">nombre</td></tr><tbody id="guoji_body"></tbody></table></div>';		
			}
