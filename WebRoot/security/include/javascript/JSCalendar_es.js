...
function createCldTabFrm(){
	...
	_TD.innerHTML = "<table cellspacing=0 cellpadding=0 class=calendar style='border:0px solid;width:100%'>"
		+ "<tr><td title=   'Fecha de hoy'style='cursor:hand'onclick=\"parent._bHaveSelectNewValue=true;parent.setTargetFormaValue(" + _dftD.getFullYear() + "," + (_dftD.getMonth() + 1) + "," + _dftD.getDate() + ");parent.hideCldTabFrm();return;parent.fillCldTabFrm(" + _dftD.getFullYear() + "," + _dftD.getMonth() + "," + _dftD.getDate() + ");\">"
		+ "<b>&nbsp;<img src='" + _imgBgToday1.src + "' width=30px>今天 : " + _dftD.getFullYear() + "-" + (_dftD.getMonth() + 1) + "-" + _dftD.getDate()
		+ "</td>"
		+ "<td width=30px>"
		+ "<img style='cursor:hand' onclick=parent.resetTargetValue() title='Fecha claro' src='" + _imgReset1.src + "' onmouseover=this.src='" + _imgReset2.src + "' onmouseout=this.src='" + _imgReset1.src + "'></td>"
...
function whenMouseOverDateItem(){
	...
	if(e.tagName == "TD"){
		...
		e.title = "fecha y hora actual: " + sCurDate;
...
function checkTime(obj){
	...
	if((tValue>59 || tValue<0 || tValue=="" || !isNum(tValue))&& obj== tMin  ){
			alert("En el momento en que introduzca el formato no es así, comprobar");
			tMin.focus();
	}
	if((tValue>23 || tValue<0 || tValue=="" || !isNum(tValue))&& obj== tHour ){
			alert("Formato de hora que ha introducido no es así, comprobar")
			tHour.focus();
			}	
	}
