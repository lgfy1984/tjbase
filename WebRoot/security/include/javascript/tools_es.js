...
//      return false;
    if(oneChar<"0"||oneChar>"9"){
      alert("columna‘"+strName+"’Sólo rellene con un entero mayor que cero");
      return false;
    }
  }
  return true;
...
function emailVerify(objEmail,blnEmpty,strName){
  ...
  if((emailAdd==null || emailAdd=="") && blnEmpty==false){
	  alert("Parte‘"+strName+"’No pueda estar vacío。");
	  objEmail.focus();
	  return false;
	  }
 if( emailAdd!="" && !check_email(emailAdd)){
	  	alert("Parte‘"+strName+"’El formato de columna no es correcto! consulte el siguiente el formato siguiente:zonghua@126.com");
		objEmail.focus();
		return false;
	  }
  return true;
...
//校验电话号码录入的有效性,有效返回true,无效返回false
	...
	if(strText=="" && blnEmpty==false ){
		alert("Parte ‘"+strName+"’daba rellenar!");
	  	objText.focus();
      	return false;
	}else if(strText!=""){
		if(strText.length > intLength){
			alert("Parte‘"+strName+"’supera la longitud específica ("+intLength+"símbolos de caracteres)， corrije。");
	  		objText.focus();
      		return false;
		}
		sStr=/[0-9]/g;
		...
		if(bln==null){
			alert("La columna‘"+strName+"’sólo pueda formar con números!");
	  		objText.focus()
      		return false;
		}
	}
  ...
  for(i=0;i<strText.length;i++){
    if(strText.charCodeAt(i)!=32){
      if(strText.charCodeAt(i)<65 || ( strText.charCodeAt(i)>90 && strText.charCodeAt(i)<97 )  ||  strText.charCodeAt(i)>122 ){
        alert("Parte‘"+strName+"’Sólo rellene con las letras。");
        objText.focus();
        return false;
      }
    }
...
//校验纯字母和数字文本录入的有效性,有效返回true,无效返回false
  var strText=objText.value;
  if(strText=="" && blnEmpty==false){
    alert("Parte‘"+strName+"’no pueda estar en vacío。");
    objText.focus();
    return false;
  }
  var i;
  for(i=0;i<strText.length;i++){
    if(strText.charCodeAt(i)!=32){
      if(strText.charCodeAt(i)<48 || ( strText.charCodeAt(i)>57 && strText.charCodeAt(i)<65 )  || ( strText.charCodeAt(i)>90 && strText.charCodeAt(i)<97 )  ||  strText.charCodeAt(i)>122 ){
        alert("Parte‘"+strName+"’sólo rellene letras y números。");
        objText.focus();
        return false;
      }
    }
...
//校验纯数字文本录入的有效性,有效返回true,无效返回false
  ...
  for(i=0;i<strText.length;i++){
    if((strText.charCodeAt(i)<48 || strText.charCodeAt(i)>57)){
      alert("Parte‘"+strName+"’sólo se rellene con números。");
      objText.focus();
      return false;
    }
  }
...
//校验带小数数字文本录入的有效性,有效返回true,无效返回false
  ...
  if( point==-1){
    if(strText.length > totalLen-decimalLen){
      alert("Parte‘"+strName+"’es demasiado grande en columna , corije。");
      objText.focus();
      return false;
    }
  }else{
    ...
    if(strText.length-(point+1) > decimalLen){
      alert("Parte‘"+strName+"’ se introduzca el número de decimales sólo es el"+decimalLen+" bit,corríjan。");
      objText.focus();
      return false;
    }
  }
...
//校验关联日期的有效性,有效返回true,无效返回false
  ...
  if(blnEmpty){
    ...
    if(strStart==""){
      alert("Parte‘"+strStartName+"’no pueda estar en vacío。");
      objStart.focus();
      return false;
    }
    if(strEnd==""){
      alert("Parte‘"+strEndName+"’no pueda estar en vacío。");
      objEnd.focus();
      return false;
    }
  }
  ...
  if(intStart>intEnd){
    alert("Parte‘"+strStartName+"’a la fecha especificada no puede ser parte‘"+strEndName+"’más tarde la fecha especificada。");
    objStart.focus();
    return false;
  }else{
    return true;
...
if(blnNowtimeAfter && !(nowDate - 24*3600000 <= inputDate)){
			alert("¡Lo siento! parte\""+strName+"\"Ingrese el tiempo requerido después de la hora actual");
			objname.focus();
			return false;
		}
if(!blnNowtimeAfter && !(nowDate>=inputDate)){
			alert("¡Lo siento! parte\""+strName+"\"Introduzca el tiempo necesario antes de lo que ahora")
			objname.focus();
			return false;
	}
return true;
...
//校验日期的有效性,有效返回true,无效返回false
  ...
  strText=objText.value;
  if(strText=="" && blnEmpty==false){
    alert("Parte‘"+strName+"’no puede estar vacío。");
    objText.focus();
    return false;
  }

  if(strText.length!=strFormat.length){
    alert("Parte‘"+strName+"’el formato no es correcto, siga‘"+strFormat+"’introduzca el formato de fecha。");
    objText.focus();
    return false;
  }
  ...
  for(i=0;i<strText.length;){
	  ...
	  if(strFormatTemp=="MM" && (strTemp>12 || strTemp<1)){
          alert("Parte‘"+strName+"’el mes de forma incorrecta, siga las‘"+strFormat+"’introduzca el formato de fecha。");
          objText.focus();
          return false;
	  }
	  if((strFormatTemp=="dd" || strFormatTemp=="DD")  && (strTemp>31 || strTemp<1)){
          alert("Parte‘"+strName+"’la fecha de forma incorrecta, siga las‘"+strFormat+"’introduzca el formato de fecha。");
          objText.focus();
          return false;
	  }
	  if(strFormatTemp=="hh" && (strTemp>24 || strTemp<0)){
          alert("Parte‘"+strName+"’la hora de forma incorrecta, siga las‘"+strFormat+"’introduzca el formato de fecha。");
          objText.focus();
          return false;
	  }
	  if((strFormatTemp=="mm" || strFormatTemp=="ss" ) && (strTemp>59 || strTemp<0)){
          alert("Parte‘"+strName+"’de minutos o segundos, la entrada no es correcta, siga‘"+strFormat+"’introduzca el formato de fecha。");
          objText.focus();
          return false;
	  }
  	  i=i+2;
...
function SelectOrgTreeVerify(strName,blnEmpty,objname){
	...
	if(deptObj!=null && empStr!=null){
		...
		alert("Parte‘"+strName+"’no puede estar vacío, por favor seleccione el personal apropiado！‘");
...
function SelectOrgTreeAll(default_empId,objname,arrayName){
		...
		areaObj[0]=new Option("--Por favor seleccione--","");
...
function percentVerify(objname,blnPassHundred,strName){
		...
		if(objname.value/100>1 && !blnPassHundred ){
				alert("Parte\""+strName+"\"valor de entrada no puede superar el 100%");
			objname.focus();
			return false;
		}
		return true;
