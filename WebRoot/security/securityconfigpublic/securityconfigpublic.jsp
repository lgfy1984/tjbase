...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<%
		...
		var setting = {
									...
									var zNodes =[
										<%	
											for(int i = 0; i < data.getModIds().length-1; i++){
												String modId = data.getModIds()[i];
												String parentModClassId = data.getParentModClassIds()[i];
												String modName = data.getModNames()[i];
										...
										%>
											{id:"<%=modId%>", pId:0, name:"<%=modName%>", open:true, collapse:false, children:[
												<%
													for(int a = 0; a < data.getModIds().length; a++){//循环读取第二层菜单
														String Id = data.getModIds()[a];
														String parentId = data.getParentModClassIds()[a];
														String Detail = data.getModNames()[a];
														String url = request.getContextPath()+"/security/securityConfigPublic.do?verbId=search&classId="+Id;
													if(parentId != null && parentId.equals(modId)){
												%>
													{id:"<%=Id%>",pId:1, name:"<%=Detail%>", url:"<%=url%>",target:"menufr"},
												<%
														}
													}
												%>
	...
	<body>
		...
		<div id="mask"></div>
		<form name="form" method="post" target="menufr" action="security/securityConfigPublic.do">
