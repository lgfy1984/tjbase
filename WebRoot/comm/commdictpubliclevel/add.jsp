...
function saveForm(){
	...
	if(document.form.itemName.value == ""){
		alert('<bean:message key="comm.jsp.common.warn32" bundle="conf.comm.comm"/>');
	    return ;
	}
		...
		<form name="form" method="post" action="comm/commDictPublicLevel.do">
			...
			<table align="center" border="0" cellpadding="0" cellspacing="0" class="tblFill" border="0" cellpadding="0" cellspacing="0" class="tblFill">
				...
				<tr>
					...
					<td>
						<input type="text"  name="seqInLevel" id="seqInLevel" size="30" maxlength="20" onkeypress="eventOnKeyPress('comments')" value="<%=commDictPublicLevel.getSeqInLevel()%>"  readonly/>
					</td>
					<td class="tblLable" >
						<bean:message key="jsp.comments" bundle="conf.Init"/>:
					</td>
					<td>
						<input type="text"  name="comments" id="comments" size="30" maxlength="50" onkeypress="eventOnKeyPress('btnSaveForm')"  max="40" dataType="LimitB" msg="备注输入过长" value="<%=commDictPublicLevel.getComments()%>" />
				</tr>
			</table>
