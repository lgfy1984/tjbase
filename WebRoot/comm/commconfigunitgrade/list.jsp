			...
			<table  border="0" cellpadding="0" cellspacing="0" class="tblSearch" align="center">
				<tr>
					<td class="tblTitle" colspan="4">
						<bean:message key="comm.jsp.commconfigunitgrade.list.td1" bundle="conf.comm.CommMessageguoh"/>
					</td>
				</tr>
				<tr>
					<td>
						<bean:message key="comm.jsp.commconfigtrue.list.td2" bundle="conf.comm.CommMessageguoh"/>
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigUnitgrade.getItemCode()%>">
				
						<bean:message key="comm.jsp.commconfigtrue.list.td3" bundle="conf.comm.CommMessageguoh"/>
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigUnitgrade.getItemName()%>">
					
						<bean:message key="comm.jsp.commconfigtrue.list.td4" bundle="conf.comm.CommMessageguoh"/>
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('<bean:message key="comm.jsp.commconfigtrue.list.td5" bundle="conf.comm.CommMessageguoh"/>')" value="<%=commConfigUnitgrade.getInputCode()%>">
					
						<input type="button" class="btnSave"   name='<bean:message key="comm.jsp.commconfigtrue.list.td5" bundle="conf.comm.CommMessageguoh"/>' value='<bean:message key="comm.jsp.common.text401" bundle="conf.comm.comm"/>' onClick="submitQueryForm();" />
					</td>
				</tr>
			</table>
			<!--列表标题-->
				...
				<tbody id="interval_row_id">
					...
					<tr class="list_neirong">
						<td class="td_shenglue leftPadding" height="25">
							<%=commConfigUnitgrade.getItemCodeList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" style="text-align:left;padding-left:10px;">
							<%=commConfigUnitgrade.getItemNameList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" >
							<%=commConfigUnitgrade.getSeqNoList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" style="text-align:left;padding-left:10px;">
							<%=commConfigUnitgrade.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigUnitgrade.getItemCodeList()[i]%>')" alt='<bean:message key="comm.jsp.commconfigrh.list.img9" bundle="conf.comm.CommMessageguoh"/>' src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigUnitgrade.getItemCodeList()[i]%>')" alt='<bean:message key="comm.jsp.commconfigrh.list.img10" bundle="conf.comm.CommMessageguoh"/>' src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigUnitgrade.getItemCodeList()[i]%>')" alt='<bean:message key="comm.jsp.commconfigrh.list.img11" bundle="conf.comm.CommMessageguoh"/>' src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
			...
			<table width="100%" align="center" class="tblScrollFooter">
				<tr>
					<td colspan="11" align="center" class="footer">
						...
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigUnitgrade.do?verbId=query&isQuery=<%=request.getParameter("isQuery") %>" />
