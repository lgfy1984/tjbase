	...
	*****/
	function add(url,itemName,drugDictId,drugCode,minSpec,minUnits,divlevel){	
			...
			if(document.body.scrollHeight<462){
				...
				newele.style.height= document.body.offsetHeight;
			...
			*/
			var check=path.substring((path.length-9),path.length);
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:50px;margin-right:10px;"><option value="1">código fonético</option><option value="2">código</option><option value="3">nombre</option></select>pregunta:<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="drugCode" value="" /><input type="hidden" id="minSpec" value="" /><input type="hidden" id="minUnits" value="" /><input  width="60" type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" maxlength="30" /><input type="button"  value="Selecciona" onclick="xuanz(\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">código</td><td width="90">código fonético</td><td width="166">nombre</td><td width="166">Medida unidad minima</td><td width="166">Medida unidad</td></tr><tbody id="guoji_body"></tbody></table></div>';		
