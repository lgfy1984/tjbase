	...
	function add(url,itemCode,itemName,divlevel){	
			...
			*/
			var check=path.substring((path.length-9),path.length);
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:50px;margin-right:10px;"><option value="1">código fonético</option><option value="2">código</option><option value="3">nombre</option></select>pregunta:<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+itemCode+'\',\''+itemName+'\');" maxlength="30" /><input type="button"  value="Selecciona" onclick="xuanz(\''+itemCode+'\',\''+itemName+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">代码</td><td width="90">拼音码</td><td width="166">名称</td></tr><tbody id="guoji_body"></tbody></table></div>';		
