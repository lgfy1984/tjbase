	...
	function add(url,itemName,drugDictId,drugCode,minSpec,minUnits,divlevel){	
			...
			*/
			var check=path.substring((path.length-9),path.length);
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:50px;margin-right:10px;"><option value="1">Phonetic code</option><option value="2">Code</option><option value="3">Name</option></select>Query：<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="drugCode" value="" /><input type="hidden" id="minSpec" value="" /><input type="hidden" id="minUnits" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" maxlength="30" /><input type="button" class="button" value="choose" onclick="xuanz(\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">Code</td><td width="90">Phonetic code</td><td width="166">Name</td><td width="166">smallest unit size</td><td width="166">Smallest unit</td></tr><tbody id="guoji_body"></tbody></table></div>';		
