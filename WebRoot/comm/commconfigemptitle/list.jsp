			...
			<table border="0" cellpadding="0" cellspacing="0" class="tblSearch" align="center">
				<tr>
					<td class="tblTitle" colspan="4">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigemptitlelist.text68"/>
					</td>
				</tr>
				<tr>
					<td>
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/>:
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigEmptitle.getItemCode()%>"/>
					
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.name"/>:
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigEmptitle.getItemName()%>"/>
					
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.inputCode"/>:
						<input name="inputCode" type="text" value="<%=commConfigEmptitle.getInputCode()%>" onkeypress="eventOnKeyPress('btnSearch')"/>
				...
				<tbody id="interval_row_id">
					...
					<tr class="list_neirong">
						<td class="td_shenglue leftPadding"  height="25">
							<%=commConfigEmptitle.getItemCodeList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" style="text-align:left;padding-left:10px;">
							<%=commConfigEmptitle.getItemNameList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" >
							<%=commConfigEmptitle.getSeqNoList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" style="text-align:left;padding-left:10px;">
							<%=commConfigEmptitle.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigEmptitle.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigEmptitle.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigEmptitle.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
			...
			<table  width="100%" align="center" class="tblScrollFooter">
				<tr>
					<td colspan="11" align="center" class="footer">
						...
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigEmptitle.do?verbId=query&isQuery=<%=request.getParameter("isQuery") %>" />
