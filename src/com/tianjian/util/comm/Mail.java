...
public boolean sendout()
{
  try
  {
   ...
   if(!(mimeMsg.getContent() instanceof   Multipart)){
...
public static void main(String[] args)
{
  ...
  String mailbody = "password:666666";
  Mail themail = new Mail("smtp.sina.com");
  themail.setNeedAuth(true);
  if (themail.setSubject("Recuperación de Contraseña") == false)
   return;
  //邮件内容 支持html 如 <font color=red>欢迎光临</font> <a href=\"http://www.laabc.com\">啦ABC</a>
  if (themail.setBody(mailbody) == false)
   return;
  //收件人邮箱
  if (themail.setTo("ypf1@vip.qq.com") == false)
   return;
  //发件人邮箱
  if (themail.setFrom("saim_1@sina.com") == false)
   return;
  //设置附件
  //if (themail.addFileAffix("#######") == false)
   //return; // 附件在本地机子上的绝对路径
  themail.setNamePass("saim_1", "saim_1"); // 用户名与密码
