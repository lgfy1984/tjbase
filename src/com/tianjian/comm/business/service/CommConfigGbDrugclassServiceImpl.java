...
import com.tianjian.comm.dao.ICommConfigGbDrugclassDAO;
import com.tianjian.comm.dao.ICommonDAO;
import com.tianjian.comm.struts.form.CommConfigGbDrugclassForm;


public class CommConfigGbDrugclassServiceImpl implements ICommConfigGbDrugclassService {

	private ICommConfigGbDrugclassDAO commConfigGbDrugclassDAO;
	private ICommonDAO commonDAO;
	public ICommConfigGbDrugclassDAO getCommConfigGbDrugclassDAO(){
		return commConfigGbDrugclassDAO;
	}
	public void setCommConfigGbDrugclassDAO(ICommConfigGbDrugclassDAO commConfigGbDrugclassDAO){
		this.commConfigGbDrugclassDAO = commConfigGbDrugclassDAO;
	}
	...
	public void updateInit(CommConfigGbDrugclassForm form){
		...
		data = commConfigGbDrugclassDAO.getByItemCode(form.getItemCodeHidden());
	...
	public void save(CommConfigGbDrugclassForm form){
		...
		commConfigGbDrugclassDAO.save(data);
	}


	public void delete(CommConfigGbDrugclassForm form){
		CommConfigGbDrugclass data = new CommConfigGbDrugclass();
		data = commConfigGbDrugclassDAO.getByItemCode(form.getItemCode());
		commConfigGbDrugclassDAO.delete(data);
		this.delete(form.getItemCode());
	}
	public void delete(String itemCode){
		List<?> ls =  commConfigGbDrugclassDAO.getAllByParentId(itemCode);
	...
	public void update(CommConfigGbDrugclassForm form){
		...
		data = commConfigGbDrugclassDAO.getByItemCode(form.getItemCode());
		this.setData(form, data);
		commConfigGbDrugclassDAO.update(data);
	}
	public CommConfigGbDrugclass checkData(CommConfigGbDrugclassForm form){
		return this.commConfigGbDrugclassDAO.getByItemCode(form.getItemCode());
	}	
	...
	public int getCount(String itemCode, String itemName, String inputCode){
		return this.commConfigGbDrugclassDAO.getCount(itemCode, itemName, inputCode);
	}
	public void getSearch(CommConfigGbDrugclassForm form, int curCount, int quChuCount){
		...
		List<?> list = this.commConfigGbDrugclassDAO.getData(form.getItemCode().trim(), form.getItemName().trim(), form.getInputCode().trim(),order, curCount, quChuCount);
		if(list!=null && list.size()>0){
			...
			for(int i=0; i<list.size(); i++){
				...
				parentNameList[i] =  this.transNullToString(commConfigGbDrugclassDAO.findValueByProp("CommConfigGbDrugclass","itemName","itemCode",parentIdList[i]));
	...
	public void getData(CommConfigGbDrugclassForm form){
		List<?> list = this.commConfigGbDrugclassDAO.findAllOrder();
		if (list!=null && list.size()>0){
			String[] itemCodeList = new String[list.size()];
			String[] itemNameList = new String[list.size()];
			String[] inputCodeList = new String[list.size()];
