...
public class CommConfigNationalityDAO extends HibernateDaoSupport implements  ICommConfigNationalityDAO{

	private static final Logger log = LogManager.getLogger(CommConfigNationalityDAO.class);
	
	public CommConfigNationality findById(String id){
		try{
			CommConfigNationality fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigNationality a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigNationality) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigNationality aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigNationality pub = (CommConfigNationality)l.get(0);
    ...
    public void save(CommConfigNationality commConfigCountry){
    	try {
    ...
    public void update(CommConfigNationality commConfigCountry){
    	try {
    ...
    public void delete(CommConfigNationality commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigNationality a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigNationality a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigNationality aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
