...
public class CommConfigSettypeDAO extends HibernateDaoSupport implements  ICommConfigSettypeDAO{
	
private static final Logger log = LogManager.getLogger(CommConfigSettypeDAO.class);
	
	public CommConfigSettype findById(String id){
		try{
			CommConfigSettype fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigSettype a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigSettype) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigSettype aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigSettype pub = (CommConfigSettype)l.get(0);
    ...
    public void save(CommConfigSettype commConfigSettype){
    	try {
    		System.out.println(commConfigSettype.getItemCode().length());
    		System.out.println(commConfigSettype.getComments()+"PPPPP");
    		System.out.println(commConfigSettype.getInputCode());
    		System.out.println(commConfigSettype.getItemCode());
    		System.out.println(commConfigSettype.getItemName());
    		System.out.println(commConfigSettype.getSeqNo());
        	getHibernateTemplate().save(commConfigSettype);
    ...
    public void update(CommConfigSettype commConfigSettype){
    	try {
        	getHibernateTemplate().update(commConfigSettype);
    ...
    public void delete(CommConfigSettype commConfigSettype){
    	try {
        	getHibernateTemplate().delete(commConfigSettype);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigSettype a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigSettype a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigSettype aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
