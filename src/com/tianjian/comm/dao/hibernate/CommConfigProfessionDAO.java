...
public class CommConfigProfessionDAO extends HibernateDaoSupport implements  ICommConfigProfessionDAO{

	private static final Logger log = LogManager.getLogger(CommConfigProfessionDAO.class);
	
	public CommConfigProfession findById(String id){
		try{
			CommConfigProfession fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigProfession a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigProfession) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigProfession aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigProfession pub = (CommConfigProfession)l.get(0);
    ...
    public void save(CommConfigProfession commConfigCountry){
    	try {
    ...
    public void update(CommConfigProfession commConfigCountry){
    	try {
    ...
    public void delete(CommConfigProfession commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigProfession a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigProfession a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigProfession aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
