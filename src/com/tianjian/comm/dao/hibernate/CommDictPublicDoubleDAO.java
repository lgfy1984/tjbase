...
public class CommDictPublicDoubleDAO extends HibernateDaoSupport implements  ICommDictPublicDoubleDAO{

	private static final Logger log = LogManager.getLogger(CommDictPublicDoubleDAO.class);
	
	public CommDictPublicDouble findById(String id){
		try{
			CommDictPublicDouble fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommDictPublicDouble a where a.id = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommDictPublicDouble) ls.get(0);
			}
    ...
    public void save(CommDictPublicDouble commDictPublicDouble){
    	try {
        	getHibernateTemplate().save(commDictPublicDouble);
    ...
    public void update(CommDictPublicDouble commDictPublicDouble){
    	try {
        	getHibernateTemplate().update(commDictPublicDouble);
    ...
    public void delete(CommDictPublicDouble commDictPublicDouble){
    	try {
        	getHibernateTemplate().delete(commDictPublicDouble);
    ...
    public List<?> getData(String id,String classCode, String dictCode, String dictValue, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommDictPublicDouble a  where 1=1 ";
...
//	    	System.out.println("------------------"+sql);
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommDictPublicDouble aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
