...
public class CommConfigGbDrugclassDAO extends HibernateDaoSupport implements ICommConfigGbDrugclassDAO {
   
	private static final Logger log = LogManager.getLogger(CommConfigGbDrugclassDAO.class);
	//++++++++++++++++++++++++保存+++++++++++++++++++++++
	public void save(CommConfigGbDrugclass commConfigGbDrugclass) {
		try{
            getHibernateTemplate().save(commConfigGbDrugclass);
	...
	//++++++++++++++++++++++++更新+++++++++++++++++++++++
	public void update(CommConfigGbDrugclass commConfigGbDrugclass) {
		try{
            getHibernateTemplate().update(commConfigGbDrugclass);
	...
	//++++++++++++++++++++++++删除+++++++++++++++++++++++
	public void delete(CommConfigGbDrugclass commConfigGbDrugclass) {
		try{
            getHibernateTemplate().delete(commConfigGbDrugclass);
	...
	public List<?> findAllOrder(){
		...
		try {
			String sql = " select "
				...
				+ " a.comments, "
				+ " a.seqNo "
				+ " from CommConfigGbDrugclass a order by a.seqNo,a.itemCode asc";
	...
	public CommConfigGbDrugclass getByItemCode(String itemCode){
		try{
			CommConfigGbDrugclass info = null;
	    	List<?> ls = getHibernateTemplate().find(" from CommConfigGbDrugclass a where a.itemCode = ? ", itemCode);
	        if(ls != null && ls.size() > 0){
	        	info = (CommConfigGbDrugclass)ls.get(0);
	        }
	...
	public List<?> getAllByParentId(String parentId){
		try{
	    	List<?> ls = getHibernateTemplate().find(" from CommConfigGbDrugclass a where a.parentId = ? ", parentId); 
