...
public class CommConfigYesDAO extends HibernateDaoSupport implements ICommConfigYesDAO {

	private static final Logger log = LogManager.getLogger(CommConfigYesDAO.class);
	/**
	 * 通过主键获取纪录
	 * @param id
	 * @return
	...
	public CommConfigYes findById(String id) {
		CommConfigYes data=null;
		try{
			List<?> l=this.getHibernateTemplate().find("from CommConfigYes a where a.itemCode = ? ", id);
			if(l!=null&&l.size()>0){
				data=(CommConfigYes) l.get(0);
			}
	...
	public List<?> findAll() {
		try{
			Query q=getSession().createQuery("from CommConfigYes a");
			List<?> l=q.list();
	...
	public String getNameById(String id) {
		...
		try{
			List<?> l= this.getHibernateTemplate().find("from CommConfigYes a where a.itemCode=?",id);
			if(l!=null&&l.size()>0){
				CommConfigYes commConfigYes= (CommConfigYes) l.get(0);
				name=commConfigYes.getItemName();
			}
	...
	public void save(CommConfigYes commConfigYes) {
		try{
			this.getHibernateTemplate().save(commConfigYes);
	...
	public void update(CommConfigYes commConfigYes) {
		try{
			this.getHibernateTemplate().update(commConfigYes);
	...
	public void delete(CommConfigYes commConfigYes) {
		try{
			this.getHibernateTemplate().delete(commConfigYes);
	...
	public List<?> getData(String itemCode, String itemName, String inputCode,
		...
		try {
	    	...
	    	sql += " from CommConfigYes a  where 1=1 ";
