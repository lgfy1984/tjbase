...
public class CommConfigNormalDAO extends HibernateDaoSupport implements ICommConfigNormalDAO {

	private static final Logger log = LogManager.getLogger(CommConfigNormalDAO.class);
	/**
	 * 通过主键获取纪录
	 * @param id
	 * @return
	...
	public CommConfigNormal findById(String id) {
		CommConfigNormal data=null;
		try{
			List<?> l=this.getHibernateTemplate().find("from CommConfigNormal a where a.itemCode = ? ", id);
			if(l!=null&&l.size()>0){
				data=(CommConfigNormal) l.get(0);
			}
	...
	public List<?> findAll() {
		try{
			Query q=getSession().createQuery("from CommConfigNormal a");
			List<?> l=q.list();
	...
	public String getNameById(String id) {
		String name="";
		try{
			List<?> l= this.getHibernateTemplate().find("from CommConfigNormal a where a.itemName=?",id);
			if(l!=null&&l.size()>0){
				CommConfigNormal commConfigNormal= (CommConfigNormal) l.get(0);
				name=commConfigNormal.getItemName();
			}
	...
	public void save(CommConfigNormal commConfigNormal) {
		try{
			this.getHibernateTemplate().save(commConfigNormal);
	...
	public void update(CommConfigNormal commConfigNormal) {
		try{
			this.getHibernateTemplate().update(commConfigNormal);
	...
	public void delete(CommConfigNormal commConfigNormal) {
		try{
			this.getHibernateTemplate().delete(commConfigNormal);
	...
	public List<?> getData(String itemCode, String itemName, String inputCode,
		...
		try {
	    	...
	    	sql += " from CommConfigNormal a  where 1=1 ";
