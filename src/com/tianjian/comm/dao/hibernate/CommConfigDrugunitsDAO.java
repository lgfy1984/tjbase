...
public class CommConfigDrugunitsDAO extends HibernateDaoSupport implements ICommConfigDrugunitsDAO {
	private static final Logger log = LogManager.getLogger(CommConfigDrugunitsDAO.class);
	//++++++++++++++++++++++++保存+++++++++++++++++++++++
	public void save(CommConfigDrugunits commConfigDrugunits) {
		// TODO Auto-generated method stub
		try{
            getHibernateTemplate().save(commConfigDrugunits);
	...
	//++++++++++++++++++++++++更新+++++++++++++++++++++++
	public void update(CommConfigDrugunits commConfigDrugunits) {
		// TODO Auto-generated method stub
		try{
            getHibernateTemplate().update(commConfigDrugunits);
	...
	//++++++++++++++++++++++++删除+++++++++++++++++++++++
	public void delete(CommConfigDrugunits commConfigDrugunits) {
		// TODO Auto-generated method stub
		try{
            getHibernateTemplate().delete(commConfigDrugunits);
	...
	public CommConfigDrugunits getByItemCode(String itemCode){
		try{
			CommConfigDrugunits info = null;
	    	List<?> ls = getHibernateTemplate().find(" from CommConfigDrugunits a where a.itemCode = ? ", itemCode);
	        if(ls != null && ls.size() > 0){
	        	info = (CommConfigDrugunits)ls.get(0);
	        }
	...
	public CommConfigDrugunits getByItemName(String itemName){
		try{
			CommConfigDrugunits info = null;
	    	List<?> ls = getHibernateTemplate().find(" from CommConfigDrugunits a where a.itemName = ? ", itemName);
	        if(ls != null && ls.size() > 0){
	        	info = (CommConfigDrugunits)ls.get(0);
