...
public class CommConfigUnittypeDAO extends HibernateDaoSupport implements  ICommConfigUnittypeDAO{

	private static final Logger log = LogManager.getLogger(CommConfigUnittypeDAO.class);
	
	public CommConfigUnittype findById(String id){
		try{
			CommConfigUnittype fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigUnittype a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigUnittype) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigUnittype aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigUnittype pub = (CommConfigUnittype)l.get(0);
    ...
    public void save(CommConfigUnittype commConfigUnittype){
    	try {
        	getHibernateTemplate().save(commConfigUnittype);
    ...
    public void update(CommConfigUnittype commConfigUnittype){
    	try {
        	getHibernateTemplate().update(commConfigUnittype);
    ...
    public void delete(CommConfigUnittype commConfigUnittype){
    	try {
        	getHibernateTemplate().delete(commConfigUnittype);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigUnittype a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigUnittype a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigUnittype aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
