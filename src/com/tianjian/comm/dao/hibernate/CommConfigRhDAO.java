...
public class CommConfigRhDAO extends HibernateDaoSupport implements  ICommConfigRhDAO{

	private static final Logger log = LogManager.getLogger(CommConfigRhDAO.class);
	
	public CommConfigRh findById(String id){
		try{
			CommConfigRh fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigRh a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigRh) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigRh aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigRh pub = (CommConfigRh)l.get(0);
    ...
    public void save(CommConfigRh commConfigCountry){
    	try {
    ...
    public void update(CommConfigRh commConfigCountry){
    	try {
    ...
    public void delete(CommConfigRh commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigRh a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigRh a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigRh aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
