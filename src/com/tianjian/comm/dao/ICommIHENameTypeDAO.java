package com.tianjian.comm.dao;

import java.util.List;
import com.tianjian.comm.bean.CommIHENameType;
 

public interface ICommIHENameTypeDAO {
   
    public CommIHENameType findById(String id);
    
    public List<?> findAll();
    
//	public String getItemName(String id);
	
    public void save(CommIHENameType commConfigAbo);
    
    public void update(CommIHENameType commConfigAbo);
    
    public void delete(CommIHENameType commConfigAbo);
    
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize);
    
    public int getCount(String itemCode, String itemName, String inputCode, String seqNo);
    
    public Long seqNoMaker(String nameOfTable);
	  
}